...
### Save Sankey Plot
pdf(paste0("./figures/single_cell/",params.ProjectName,"SankeyPlot.pdf"), width = 20, height = 15)
pl
dev.off()
...
for(j in 1:length(params.resolutions)){
    ...
    write.table(get(paste0("MarkersRes",params.resolutions[j])), file = paste0("./markers/single_cell/",ifelse(params.integration ==T,paste0(" Integrated_", params.integrationMethod,"_"), "Unintegrated"),"MarkersRes",params.resolutions[j],".tsv"), sep = "\t", quote = F,row.names = F)
    rm(markertemp)
}
```
...
for (name in params.SampleNames) {
  rows <- which(grepl(name, rownames(cell_cluster_idents)))
  tmp <- cell_cluster_idents[rows,]
  rownames(tmp) <- lapply(rownames(tmp), slice_id)
  write.csv(tmp, file = paste0("projections/seurat_clusters.", name,".csv"), col.names = NA, quote = FALSE, row.names = TRUE)
}
}else {
